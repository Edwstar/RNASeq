...
if (os.path.isfile(opts.input)):
    ...
    try:
        ...
        for line in fin:
            if line[0] != '#':
                ...
                if (len(lineLst) != 2):
                    print
                    "Error: Text file with sample ID and path invalid (%s)" % (line.strip())
                    exit(1)
                if lineLst[0] in samples:
                    print
                    "Error: Sample ID duplicated (%s)" % (lineLst[0])
                    exit(1)
                if not os.path.isfile(lineLst[1]):
                    print
                    "Error: GTF file not found (%s)" % (lineLst[1])
                    exit(1)
                samples.append(lineLst)
    except IOError:
        print
        "Error: List of .gtf files, %s, doesn't exist" % (opts.input)
        exit(1)
else:
    # gtfList = False
    ...
    if not os.path.isdir(opts.input):
        ...
        " "
        print
        "Error: sub-directory '%s' not found!" % (opts.input)
        sys.exit(1)
...
if len(samples) == 0:
    ...
    " "
    print
    "Error: no GTF files found under ./%s !" % (opts.input)
    sys.exit(1)
...
def getGeneID(s, ctg, tid):
    ...
    if r:
        if not r.group(1).startswith('MSTRG'):
            return r.group(1)
